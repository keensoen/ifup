...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        //dd(json_encode($this->getKeywordFrequency($prayerData)));

        $month = date('m', strtotime(Carbon::today()));
        $week = date('Y-m-d', strtotime(Carbon::today()->subDay(7)));
        $today = date('Y-m-d', strtotime(Carbon::today()));

        $commentModel = MemberComment::whereMonth('created_at', $month);
        $prayerModel = PrayerRequest::whereMonth('created_at', $month);

        $comments = authRole($commentModel, auth()->user());
        $prayerRequest = authRole($prayerModel, auth()->user());

        if(auth()->user()->hasRole('super-admin')) {
            $totalMembers = Member::whereMonth('created_at', $month)->count();
            ...
            //dd($this->removeCommonWords($prayerData));

            foreach (MemberComment::whereMonth('created_at', $month)->get(['comment']) as $key => $item) {
                $commentData .= $item->comment.' ';
            }

            $upcomingBirthday = Member::whereMonth('created_at', $month)->whereMonth('birthday', $month)->get();
            $absents = Member::doesntHave('attendances')->whereMonth('created_at', $month)->whereBetween('created_at', [$week, $today])->get();
            ...
            foreach(MemberComment::whereMonth('created_at', $month)->whereOrganizationId(auth()->user()->organization_id)->get(['comment']) as $item) {
                $commentData .= $item->comment.' ';
            }
    ...
    public function getKeywordFrequency($data) {
        $allWordArray = str_word_count(strip_tags($data), 1);
        $totalAllowWordArray = count($allWordArray);
        $wordCount = array_count_values($allWordArray);
        ...
        foreach ($wordCount as $word => $val) {
            $percentageCount[$word] = ['name' => $word, 'frequecy' => $val, 'percentage' => number_format($val/$totalAllowWordArray * 100, 2)]; 
        }
